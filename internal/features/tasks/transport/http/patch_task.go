...
import (
	...
	core_http_types "github.com/Dex564/golang-todoapp/internal/core/transport/http/types"
)
...
func (h *TasksHTTPHandler) PatchTask(rw http.ResponseWriter, r *http.Request) {
	...
	var patchTaskRequest PatchTaskRequest
	if err := core_http_request.DecodeAndValidateRequest(r, &patchTaskRequest); err != nil {
		responseHandler.ErrorResponse(err, "failed to decode and validate HTTP request")
		return
	}

	// log.Debug("task values: ", zap.Bool("completed_set", patchTaskRequest.Completed.Set), zap.Bool("completed_val", *patchTaskRequest.Completed.Value))
	taskPatch := taskPatchFromRequest(patchTaskRequest)
